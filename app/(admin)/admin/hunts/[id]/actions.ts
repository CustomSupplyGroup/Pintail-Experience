...
export type HuntState = { ok: boolean; message: string };
...
export async function updateHunt(
  ...
  if (!id) return { ok: false, message: "Missing hunt id." };
  ...
  const planning_status = (
    PLANNING.includes(planningRaw as PlanningStatus)
      ? planningRaw
      : "scoping"
  ) as PlanningStatus;
  ...
  if (error) {
    console.error("hunt update failed:", error.message);
  ...
  revalidatePath("/");
  revalidatePath("/home");
  return { ok: true, message: "Hunt saved." };
}
