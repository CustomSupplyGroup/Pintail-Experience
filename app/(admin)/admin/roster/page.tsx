...
export default async function RosterPage() {
  ...
  if (tripError) {
    return (
      <div>
        ...
        <EmptyState>Couldn&apos;t load the trip: {tripError}</EmptyState>
      </div>
    );
  }
  if (!trip) {
    return (
      <div>
        <PageHeader title="Roster" />
        <EmptyState>No active trip yet.</EmptyState>
      </div>
    );
  }
  ...
  const people = (rows ?? [])
    .map((r) => ({
      ...
      full_name: (r.users as { full_name: string | null } | null)?.full_name ?? null,
